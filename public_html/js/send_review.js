...
let displayAlert = (error, prod_id) => {
    ...
    if (!!error) {
        ...
        alert_to_show
            .children("p")
            .text("La review non è stata inserita correttamente");
    } else {
...
const showReview = (prod_id) => {
    ...
    $("#reviews" + prod_id).append(
        constructReview(author, reviewScore, reviewText)
    );
};
        ...
        .then((data) => {
            if (!data.confirmed) {
                ...
                alertDiv
                    .children("p")
                    .text(
                        "Per lasciare una review devi prima acquistare questo prodotto"
                    );

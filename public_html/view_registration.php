    ...
    <div class="text-center">
        <main class="form-signin">
            <form id="registerForm" action="registration.php" method="POST">
                ...
                <input class="form-control" placeholder="Nome" required="" type="text" id="firstname" name="firstname" value="<?php if (isset($restoring) && isset($restoring['firstname'])) echo $restoring["firstname"] ?>" pattern="<?php echo substr($fstname_reg, 1, strlen($fstname_reg) - 2) ?>">

                <label for="lastname" class="visually-hidden">*Cognome:</label>
                <input class="form-control" placeholder="Cognome" required="" type="text" id="lastname" name="lastname" value="<?php if (isset($restoring) && isset($restoring['lastname'])) echo $restoring["lastname"] ?>" pattern="<?php echo substr($lastname_reg, 1, strlen($lastname_reg) - 2) ?>">
                ...
                <input class="form-control" placeholder="Username" type="text" id="username" name="username" value="<?php if (isset($restoring) && isset($restoring['username'])) echo $restoring["username"] ?>" pattern="<?php echo substr($username_reg, 1, strlen($username_reg) - 2) ?>">

                <label for="address" class="visually-hidden">Indirizzo:</label>
                <input class="form-control" placeholder="Indirizzo" type="text" id="address" name="address" value="<?php if (isset($restoring) && isset($restoring['address'])) echo $restoring["address"] ?>" pattern="<?php echo substr($address_reg, 1, strlen($address_reg) - 2) ?>">

                <label for="phone" class="visually-hidden">Telefono:</label>
                <input class="form-control" placeholder="Telefono" type="text" id="phone" name="phone" value="<?php if (isset($restoring) && isset($restoring['phone'])) echo $restoring["phone"] ?>" pattern="<?php echo substr($phone_reg, 1, strlen($phone_reg) - 2) ?>">
